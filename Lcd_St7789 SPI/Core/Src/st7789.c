...
#define WAIT_SPI while (ST7789_SPI_DEVICE->SR & SPI_SR_BSY) {};
...
generic_st7789[] = {                // Init commands for 7789 screens
    ...
    ST7789_CASET  , 4,              //  5: Column addr set, 4 args, no delay:
      ...
      0,              //     XSTART = 0
      0,                                  //240>>8
      240,            //     XEND = 240     240&0xFF
    ST7789_RASET  , 4,              //  6: Row addr set, 4 args, no delay:
      0x00,
      0,              //     YSTART = 0
      0,                                  //320>>8
      240,            //     YEND = 240   = 320&0xFF
...
static void ST7789_SetAddressWindow(uint16_t x1, uint16_t y1, uint16_t x2, uint16_t y2)
{
...
void ST7789_Init(void){
  ...
  RST_LOW; LL_mDelay(10); RST_HIGH;  //Hard-reset
...
void lcd_clear(uint16_t color)
{
    ...
    ST7789_WriteSPI(0);
    ST7789_WriteSPI(0);
    ST7789_WriteSPI(0X01);
    ST7789_WriteSPI(0X40);
    ...
    for (i = 0; i < 240; i++)
    {
        for (j = 0; j < 240; j++)
        {
            ...
            ST7789_WriteSPI(color);
        }
    }
    //lcd_display_on(); /*  open LCD Show  */
}
